...
var bitcore = require('litecore-lib'); //@ntr upgrade Insight2-3
...
var unityCoin = 1e-8;
var bitCoin = 1e8; // 100,000,000 satoshis
...
WalletController.prototype.sendTx = function (req, res) {
	...
	try {
			var amountBN = BigNumber(amount).multipliedBy(bitCoin).integerValue();
...
WalletController.prototype.createUnsigedRawTransaction = function (req, res) {
	...
	try {
		var amountBN = BigNumber(amount).multipliedBy(bitCoin).integerValue();
      	var customFeeBN = BigNumber(customFee).multipliedBy(bitCoin).integerValue();
      	var toAddr = new bitcore.Address(toAddrString);
      	var fromAddr = new bitcore.Address(fromAddrString);
    } catch(e) {
      	return self.common.handleErrors({
	...
	//Check Dust inputs
	if (amountBN.isLessThan(54600)) {
		return self.common.handleErrors({
			message: 'Sending amount is less than DUST_AMOUNT [54600 sat]',
			code: 1
		}, res);
	}
	...
	this.node.estimateFee(nConfirmExpect, function (err, feeRateEstimate) {
		if (err) {
			...
			if (feeRateEstimate > 0) {
				_feePerKb = BigNumber(feeRateEstimate).multipliedBy(bitCoin).toNumber();
			}
		}
		...
		if (customFeeBN.isGreaterThan(0)) {
            ...
            if(expectedFee < 1000) expectedFee = 1000;
		}
		...
		self.node.getAddressUnspentOutputs(fromAddrString, true, function (err, utxos) {
            ...
            if(expected_fee < 1000) expected_fee = 1000;
...
WalletController.prototype.createUnsigedRawTransactionMulti = function (req, res) {
	...
	toAddrs.forEach(function(outAddr){
		...
		if (err) return self.common.handleErrors(err, res);
		try{
			var amountBN = BigNumber(outAddr.amount).multipliedBy(bitCoin).integerValue();
			...
			if (amountBN.isLessThan(54600)) {
				return self.common.handleErrors({
					message: 'One of the sending outputs amount is less than DUST_AMOUNT [54600 sat]',
					code: 1
				}, res);
			}
		} catch (e) {
